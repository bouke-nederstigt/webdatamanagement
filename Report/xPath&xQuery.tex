...
\item[For each movie that has at least two actors, list the title and firs two actors, and an empty "et-al" element if the movie has additional actors] \hfill \\
  \begin{verbatim}
	<results>{
	let $ms := doc("movies/movies_alone.xml"), $as := doc("movies/artists_alone.xml")
	for $movie in $ms//movie 
